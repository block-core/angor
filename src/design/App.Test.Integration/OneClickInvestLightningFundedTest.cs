...
public class OneClickInvestLightningFundedTest
{
    ...
    public async Task LightningInvoice_ManualPay_ReachesSuccessScreen()
    {
        ...
        {
            Dispatcher.UIThread.RunJobs();
            if (pf.OnChainAddress != null)
                break;
            await Task.Delay(250);
        }
        pf.OnChainAddress.Should().NotBeNullOrEmpty(
            "receive address should be generated before switching to Lightning tab");
        Log($"[4b] Receive address ready: {pf.OnChainAddress}");
        ...
        while (DateTime.UtcNow < invoiceDeadline)
        {
            ...
            if (pf.ErrorMessage != null)
                throw new Exception($"Lightning flow errored: {pf.ErrorMessage}");

            await Task.Delay(500);
        }

        // Read the full invoice from the VM — the UI TextBlock truncates it via TextTrimming
        var bolt11Invoice = pf.LightningInvoice;
        ...
        while (DateTime.UtcNow < successDeadline)
        {
            ...
            if (pf.ErrorMessage != null)
            {
                Log($"[8] ERROR: {pf.ErrorMessage}");
                Assert.Fail($"Investment flow failed at step 8: {pf.ErrorMessage}");
            }
        ...
        reachedSuccess.Should().BeTrue(
            ...
            $"Last status: '{lastStatus}', IsProcessing: {pf.IsProcessing}, Error: {pf.ErrorMessage ?? "none"}");

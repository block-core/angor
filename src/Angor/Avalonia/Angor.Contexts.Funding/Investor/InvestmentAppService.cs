...
{
    public Task<Result<InvestmentDraft>> CreateInvestmentDraft(string sourceWalletId, ProjectId projectId, Amount amount, DomainFeerate feerate)
    {
        return mediator.Send(new CreateInvestment.CreateInvestmentTransactionRequest(sourceWalletId, projectId, amount, feerate));
    }

    public Task<Result<Guid>> SubmitInvestment(string sourceWalletId, ProjectId projectId, InvestmentDraft draft)
    {
        return mediator.Send(new RequestInvestmentSignatures.RequestFounderSignaturesRequest(sourceWalletId, projectId, draft));
    }

    public Task<Result<IEnumerable<InvestedProjectDto>>> GetInvestorProjects(string walletId)
    {
        return mediator.Send(new Investments.InvestmentsPortfolioRequest(walletId));
    }

    public Task<Result> ConfirmInvestment(string investmentId, string walletId, ProjectId projectId)
    {
        return mediator.Send(new PublishInvestment.PublishInvestmentRequest(investmentId, walletId, projectId));
    }
    
    public Task<Result<IEnumerable<PenaltiesDto>>> GetPenalties(string walletId)
    {
    ...
    public Task<Result<InvestorProjectRecoveryDto>> GetInvestorProjectRecovery(string walletId, ProjectId projectId)
    {
    ...
    public Task<Result<RecoveryTransactionDraft>> BuildRecoverInvestorFunds(string walletId, ProjectId projectId, DomainFeerate feerate)
    {
        return mediator.Send(new RecoverFunds.RecoverFundsRequest(walletId, projectId, feerate));
    }

    // Investor/Manage Funds
    public Task<Result<ReleaseTransactionDraft>> BuildReleaseInvestorFunds(string walletId, ProjectId projectId, DomainFeerate feerate)
    {
        return mediator.Send(new ReleaseFunds.ReleaseFundsRequest(walletId, projectId, feerate));
    }

    // Investor/Manage Funds
    public Task<Result<EndOfProjectTransactionDraft>> BuildClaimInvestorEndOfProjectFunds(string walletId, ProjectId projectId, DomainFeerate feerate)
    {
    ...
    {
        return mediator.Send(new PublishTransaction.PublishTransactionRequest(walletId, null, draft));
    }

    public Task<Result<string>> SubmitTransactionFromDraft(string walletId, ProjectId projectId, TransactionDraft draft)
    {

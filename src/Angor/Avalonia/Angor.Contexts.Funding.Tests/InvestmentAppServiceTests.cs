using Angor.Sdk.Common;
using Angor.Sdk.Funding.Founder;
using Angor.Sdk.Funding.Investor;
using Angor.Sdk.Funding.Shared;
using Angor.Sdk.Funding.Tests.TestDoubles;
using Angor.Shared;
using CSharpFunctionalExtensions;
using Microsoft.Extensions.DependencyInjection;
using Serilog;
using Xunit.Abstractions;
using Amount = Angor.Sdk.Funding.Projects.Domain.Amount;

namespace Angor.Sdk.Funding.Tests;

using Draft = Shared.TransactionDrafts.InvestmentDraft;

public class InvestmentAppServiceTests(ITestOutputHelper output)
{
    private static readonly WalletId TestWalletId = new("test-wallet-id-123");

    [Fact]
    public void Service_can_be_created()
    {
        // Act
        CreateInvestorSut();
    }

    [Fact(Skip = "Skipping failing test: DI for document collection / logger not configured yet.")]
    public async Task Create_investment_draft()
    {
        // Arrange
        var sut = CreateInvestorSut();
        var projectId = new ProjectId("angor1qkmmqqktfhe79wxp20555cdp5gfardr4s26wr00");
        
        // Act
        var result = await sut.CreateInvestmentDraft(TestWalletId, projectId, new Amount(12345), new DomainFeerate(123));
        
        // Assert
        Assert.True(result.IsSuccess, result.IsFailure ? result.Error : string.Empty);
    }

    [Fact(Skip = "Skipping failing test: DI for document collection / logger not configured yet.")]
    public async Task Request_investment_in_project()
    {
        // Arrange
        var sut = CreateInvestorSut();
        var projectId = new ProjectId("angor1qkmmqqktfhe79wxp20555cdp5gfardr4s26wr00");
        var investorKey = "03138f2811a0b7589d1b04a34b561f55093965757b31b239b3b201d93825455838";
        var txHex = "";
        var txId = "1b55a4706d7315ea32bfe9c004781f0dee1df3ba18fd7d4423ebad2399dcf96a";
        var draft = new Draft(investorKey)
        { SignedTxHex = txHex, TransactionId = txId, TransactionFee = new Amount(0), };

        // Act
        var result = await sut.SubmitInvestment(TestWalletId, projectId, draft);

        // Assert
        Assert.True(result.IsSuccess, result.IsFailure ? result.Error : string.Empty);
    }

    [Fact(Skip = "Skipping failing test: DI for document collection / logger not configured yet.")]
    public async Task Get_investments()
    {
        // Arrange
        var sut = CreatFounderSut();

        // Act
        var result = await sut.GetInvestments(TestWalletId, new ProjectId("angor1qatlv9htzte8vtddgyxpgt78ruyzaj57n4l7k46"));

        // Assert
        Assert.True(result.IsSuccess);
        Assert.NotEmpty(result.Value);
    }

    [Fact(Skip = "Skipping failing test: DI for document collection / logger not configured yet.")]
    public async Task Approve_investment()
    {
        // Arrange
        var sut = CreatFounderSut();
        var pendingInvestments = await sut.GetInvestments(TestWalletId, new ProjectId("angor1qatlv9htzte8vtddgyxpgt78ruyzaj57n4l7k46"));
        var pendingInvestmentDto = pendingInvestments.Value.TryFirst().GetValueOrThrow("Failed");

        // Act
        var approveResult = await sut.ApproveInvestment(TestWalletId, new ProjectId("angor1qatlv9htzte8vtddgyxpgt78ruyzaj57n4l7k46"), pendingInvestmentDto);
        
        // Assert
        Assert.True(approveResult.IsSuccess);
    }

    private IInvestmentAppService CreateInvestorSut()
    {
        var serviceCollection = new ServiceCollection();

        var logger = new LoggerConfiguration().WriteTo.TestOutput(output).CreateLogger();
        FundingContextServices.Register(serviceCollection, logger);
        serviceCollection.AddSingleton<ISeedwordsProvider>(sp => new TestingSeedwordsProvider("oven suggest panda hip orange cheap kite focus cross never tornado forget", "", sp.GetRequiredService<IDerivationOperations>()));

        var serviceProvider = serviceCollection.BuildServiceProvider();
        var projectAppService = serviceProvider.GetService<IInvestmentAppService>();

        return projectAppService!;
    }
    
    private IFounderAppService CreatFounderSut()
    {
        var serviceCollection = new ServiceCollection();

        var logger = new LoggerConfiguration().WriteTo.TestOutput(output).CreateLogger();
        FundingContextServices.Register(serviceCollection, logger);
        serviceCollection.AddSingleton<ISeedwordsProvider>(sp => new TestingSeedwordsProvider("oven suggest panda hip orange cheap kite focus cross never tornado forget", "", sp.GetRequiredService<IDerivationOperations>()));

        var serviceProvider = serviceCollection.BuildServiceProvider();
        var projectAppService = serviceProvider.GetService<IFounderAppService>();

        return projectAppService!;
    }
}
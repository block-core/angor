...
public partial class ApproveInvestmentsViewModel : ReactiveObject, IApproveInvestmentsViewModel
{
    ...
    public ApproveInvestmentsViewModel(ProjectId projectId, IFounderAppService founderAppService, UIServices uiServices)
    {
        LoadInvestments = ReactiveCommand.CreateFromTask(() =>
            {
                return uiServices.WalletRoot.GetDefaultWalletAndActivate()
                    ...
                    .Bind(wallet => GetWalletInvestments(projectId, founderAppService, wallet))
                    .Map(tuple => tuple.investments.GroupBy(investment => new InvestmentGroupKey(investment, tuple.wallet))
                        .Select(IInvestmentViewModel (grouping) => new InvestmentViewModel(grouping, () => Approve(projectId, founderAppService, uiServices, tuple.wallet, grouping.Key.Investment))));
            })
    ...
    private static Task<Result<(IWallet wallet, IEnumerable<Investment> investments)>> GetWalletInvestments(ProjectId projectId, IFounderAppService founderAppService, IWallet wallet)
    {
        var investments = founderAppService.GetInvestments(wallet.Id.Value, projectId);
        return investments.Map(x => (wallet, investments: x));
    }

    private static async Task<bool> Approve(ProjectId projectId, IFounderAppService founderAppService, UIServices uiServices, IWallet wallet, Investment investment)
    {

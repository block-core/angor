...
public class SendMoneyFlow(IWalletAppService walletAppService, UIServices uiServices) : ISendMoneyFlow
{
    public async Task SendMoney(IWallet sourceWallet)
    {
        var wizard = WizardBuilder
            ...
            .Then(sendData => new TransactionDraftViewModel(sourceWallet.Id, walletAppService, new SendAmount("Test", sendData.Amount.GetValueOrDefault(), sendData.Address!), uiServices), "Summary").NextCommand(model => model.Confirm.Enhance("Confirm"))

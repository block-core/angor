...
async function nip04Encryption(sharedSecretHex, message) {
...
async function nip04Decryption(sharedSecretHex, encryptedMessage) {
    const sharedSecret = hexToBytes(sharedSecretHex);
...
async function encryptNostr(sharedSecretHex, message) {
    try {
        return  NostrTools.nip44.encrypt(message,sharedSecretHex);
    } catch (e) {
        console.log(e)
        throw e;
    }

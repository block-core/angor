...
public class InvestmentOperations
{
    ...
    /// </summary>
    public Transaction CreateInvestmentTransaction(AngorNetwork network,InvestorContext context, long totalInvestmentAmount)
    {
        ...
        {
            investmentTransaction.Outputs.Add(stagesOutput);
        }
    ...
    public Transaction SignInvestmentTransaction(AngorNetwork network,string changeAddress, Transaction transaction, WalletWords walletWords, List<UtxoDataWithPath> utxoDataWithPaths,
        ...
        FeeEstimation fee)
    {
        ...
        return Transaction.Parse(spender.ToHex(), network);
    }
    ...
    public List<Transaction> BuildRecoverInvestorFundsTransactions(InvestorContext context, Network network, string investorReceiveAddress)
    {
        ...
        var founderStageSignatures = investmentTransaction.Outputs.AsIndexedOutputs()
            ...
            .Select((_, i) =>
            {
                ...
                return Transaction.Parse(stageTransaction.ToHex(), nbitcoinNetwork);
            });
    ...
    /// allow an investor that take back any coins left when the project end date has passed
    /// </summary>
    public Transaction RecoverFundsNoPenalty(Network network, InvestorContext context, int[] stages, Key[] seederSecrets, Script investorRecieveAddress, string investorPrivateKey)
    {
        // We'll use the NBitcoin lib because its a taproot spend
